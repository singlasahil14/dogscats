...
def hyperparams_beamsearch(range_dict, get_loss_history, beam_size=2, 
                           num_configs=5, max_epochs=70):
    import time
    f1 = open('fine_all_configs', 'w+')
    ...
    print('\n\n\n')
    for key, max_value in range_dict.iteritems():
        points = [i/float(num_configs-1) for i in range(num_configs)]
...
range_dict = {'width_shift_range': (0.15,0.35), 'height_shift_range': (0,0.2), 
              'rotation_range': (0,15), 'shear_range': (0,0.2), 
              'zoom_range': (0.4,0.6), 'channel_shift_range': (0,20)}
